                ...
                <main>
                    <div class="container-fluid">
                        ...
                        <ol class="breadcrumb mb-4">
                            <li class="breadcrumb-item active">Add New Periodical</li>
                        </ol>
                       
                        <div class="card-header"><h3 class="text-center font-weight-light my-4">Add Periodical</h3></div>
                                    <div class="card-body">
                                        <form action="addperiadical.php" method="post">
                                            <div class="form-row">
                                                
                                                
                                                <div class="col-md-6">
                                                    <div class="form-group">
                                                        <label class="small mb-1" for="periodical">periodical Title</label>
                                                        <input class="form-control py-4" id="periodical" type="text" placeholder="Enter periodical Title" name="periodicaltitle" required/>
                                                    </div>
                                                </div>
                                                <div class="col-md-6">
                                                    <div class="form-group">
                                                        <label class="small mb-1" for="inputpr">Production Release</label>
                                                        <input class="form-control py-4" id="inputpr" type="text" placeholder="Enter Production Release Year" name="pr" required/>
                                                    </div>
                                                </div>
                                            </div>
                                             <div class="form-row">
                                                ...
                                                <div class="col-md-6">
                                                    <div class="form-group">
                                                        <label class="small mb-1" for="inputpublisher">Periodical Publisher</label>
                                                        <input class="form-control py-4" id="inputpublisher" type="text" placeholder=" Enter Periodical's Publisher" name="publisher" required/>
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <div class="form-row">
                                                ...
                                                <div class="col-md-6">
                                                        ...
                                                        <input class="form-control py-4" id="author" type="text" placeholder="Enter Periodical's Author " name="author" required/>
                                                    </div>
                                            ...
                                            <div class="form-group mt-4 mb-0"><input type="submit" value="Add Periodical" name="add" class="btn btn-success"  ></div>
                                            </center>
                                        </form>
                                        <?php
                                        if(isset($_POST["add"])){
                                            ...
                                            $st_serial=$con->prepare("select serial from periadical where serial=? ");
                                            $st_serial->bind_param('i', $_POST["serial"]);
                                            $st_serial->execute();
                                            $rs_serial=$st_serial->get_result();
                                            
                                            
                                             $st_isbn=$con->prepare("select isbn from periadical where isbn=? ");
                                            $st_isbn->bind_param('s', $_POST["isbn"]);
                                            $st_isbn->execute();
                                            $rs_isbn=$st_isbn->get_result();
                                            
                                            if($rs_serial->num_rows==1 || $rs_isbn->num_rows==1){
                                                echo '<script>alert("Serial Number Or ISBN Already Exist");</script>';
                                                ...
                                                $st=$con->prepare("INSERT INTO `periadical` (`periodicalID`, `title`, `year`, `pages`, `section`, `publisherName`, `autherName`,`serial`,`isbn`) VALUES (null,?,?,?,?,?,?,?,?)");
                                                $st->bind_param("siisssis", $_POST["periodicaltitle"],$_POST["pr"],$_POST["pn"],$_POST["sc"],$_POST["publisher"],$_POST["author"],$_POST["serial"],$_POST["isbn"]);
                                                $st->execute();
                                             
                                                 echo '<script>alert("periodical Added");</script>';
                                            }
