            ...
            <div id="layoutSidenav_content">
                <main>
                    <div class="container-fluid">
                        ...
                        <div class="card-header"><h3 class="text-center font-weight-light my-4">INSERT PERIODICAL BORROWING</h3></div>
                                    <div class="card-body">
                                        <form action="insertPeriodical.php" method="post">
                                            <div class="form-row">
                                                ...
                                                <div class="col-md-6">
                                                    <div class="form-group">
                                                        <label class="small mb-1" for="inputpr">Periodical ID</label>
                                                        <input class="form-control py-4" id="inputpr" type="text" placeholder="Enter Periodical ID" name="pr" required/>
                                                    </div>
                                                </div>
                                            </div>
                                        ...
                                        if(isset($_POST["insert"])){
                                            ...
                                            $st_serial=$con->prepare("select periodicalBorrow from periodicalborrow where periodicalBorrow=? ");
                                            ...
                                            $st_per=$con->prepare("select periodicalID from periadical where periodicalID=? ");
                                            $st_per->bind_param('i', $_POST["pr"]);
                                            $st_per->execute();
                                            $rs_per=$st_per->get_result();
                                            ...
                                            if($rs_serial->num_rows==1   ){
                                                echo '<script>alert("Periodical Already Borrowed");</script>';
                                                
                                            }
                                            
                                            elseif ($rs_user->num_rows==0 || $rs_per->num_rows==0 ) {
                                             echo '<script>alert("User ID is wroing Or Periodicl ID !!");</script>';
                                        }
                                                ...
                                                $st=$con->prepare("INSERT INTO `periodicalborrow` (`borrowID`, `id`, `periodicalBorrow`, `time`, `status`) VALUES (NULL, $_POST[bt], $_POST[pr], current_timestamp(), '1');");
                                                $st->execute();
